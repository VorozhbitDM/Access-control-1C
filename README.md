...
> С **v4.7** в настройках можно указать и другие конфигурации (БП, ЗУП, УНФ, Документооборот 3.0, «Другая»). Это **задел на будущее**: сам факт выбора конфигурации из списка **не означает**, что обработка уже полностью с ней работает — полноценная поддержка будет добавляться постепенно. На текущий момент **стабильная работа** гарантирована только для конфигураций из таблицы выше.
